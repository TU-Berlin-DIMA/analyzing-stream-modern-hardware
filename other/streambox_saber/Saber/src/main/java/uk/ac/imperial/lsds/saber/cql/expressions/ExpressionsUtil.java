...
public class ExpressionsUtil {
    ...
    public static int doubleToByteArray(double value, byte[] bytes, int pivot) {
        long bits = Double.doubleToLongBits(value);
        bytes[pivot + 0] = (byte) ((bits)       & 0xff);
        bytes[pivot + 1] = (byte) ((bits >>  8) & 0xff);
        bytes[pivot + 2] = (byte) ((bits >> 16) & 0xff);
        bytes[pivot + 3] = (byte) ((bits >> 24) & 0xff);
        bytes[pivot + 4] = (byte) ((bits >> 32) & 0xff);
        bytes[pivot + 5] = (byte) ((bits >> 40) & 0xff);
        bytes[pivot + 6] = (byte) ((bits >> 48) & 0xff);
        bytes[pivot + 7] = (byte) ((bits >> 56) & 0xff);
        return (pivot + 8);
    }

    public static void doubleToByteArray(double value, byte[] bytes) {
        doubleToByteArray(value, bytes, 0);
    }
