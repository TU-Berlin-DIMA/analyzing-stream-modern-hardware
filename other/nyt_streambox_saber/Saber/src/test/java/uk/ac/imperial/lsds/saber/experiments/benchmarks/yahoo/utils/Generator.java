...
public class Generator {
    ...
    private PAPIHardwareSampler[] generator_papi_samplers;
	...
	public Generator (int bufferSize, int numberOfThreads, int adsPerCampaign, long[][] ads, int coreToBind, boolean isV2) {
        this(bufferSize, numberOfThreads, adsPerCampaign, ads, coreToBind, isV2, null);
	}

	public Generator (int bufferSize, int numberOfThreads, int adsPerCampaign, long[][] ads, int coreToBind, boolean isV2, PAPIHardwareSampler[] generator_papi_samplers) {
        this.generator_papi_samplers = generator_papi_samplers;
		...
		for (int i = 0; i < workers.length; i++) {
            ...
            workers[i] = new GeneratorWorker (this, positionsList.get(i).get(0), positionsList.get(i).get(1), i + coreToBind, isV2, this.generator_papi_samplers[i]);
			//workers[i].configure();
		...
		fillNext ();
		// this(bufferSize, numberOfThreads, adsPerCampaign, ads, coreToBind, isV2);
	}
